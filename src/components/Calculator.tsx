...
/**
 * - (INPUT_MODE) -> you are giving a number
 * - (OP_MODE) -> you are giving an operation
 * - (CALC_MODE) -> you are calculating by clicking =
 * - (EMPTY_MODE) -> when there's no a given operation
 */
enum Mode {
  INPUT_MODE,
  OP_MODE,
...
const defaultSate: TYPES.StateType = {
  operand1: "",
  operand2: "",
  operation: "",
  result: "0",
...
const BackSpaceHandler = (state: TYPES.StateType) => {
  const previusResult = state.result;
...
const operationsHandler = (state: TYPES.StateType, op: TYPES.OpSymbol) => {
  switch (state.mode) {
    case Mode.OP_MODE:
...
const calculateHandler = (state: TYPES.StateType) => {
...
const reducer = (state: TYPES.StateType, action: ACTION) => {
  switch (action) {
    case ACTION.BS:
